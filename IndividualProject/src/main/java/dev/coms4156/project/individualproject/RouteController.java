...
public class RouteController {
  ...
  public ResponseEntity<?> getMajorCountFromDepartment(@RequestParam String deptCode) {
    try {
      boolean doesDepartmentExists = retrieveDepartment(deptCode).getStatusCode() == HttpStatus.OK;
      if (doesDepartmentExists) {
        Map<String, Department> departmentMapping;
  ...
  public ResponseEntity<?> identifyDepartmentChair(@RequestParam String deptCode) {
    try {
      boolean doesDepartmentExists = retrieveDepartment(deptCode).getStatusCode() == HttpStatus.OK;
      if (doesDepartmentExists) {
        Map<String, Department> departmentMapping;
  ...
  public ResponseEntity<?> addMajorToDepartment(@RequestParam String deptCode) {
    try {
      boolean doesDepartmentExists = retrieveDepartment(deptCode).getStatusCode() == HttpStatus.OK;
      if (doesDepartmentExists) {
        Map<String, Department> departmentMapping;
  ...
  public ResponseEntity<?> removeMajorFromDepartment(@RequestParam String deptCode) {
    try {
      boolean doesDepartmentExists = retrieveDepartment(deptCode).getStatusCode() == HttpStatus.OK;
      if (doesDepartmentExists) {
        Map<String, Department> departmentMapping;
  ...
  public ResponseEntity<?> dropStudentFromCourse(
      @RequestParam String deptCode,
      @RequestParam int courseCode
  ) {
    try {
   ...
   * count of a course identified by department code and course code.
   ...
   * @param courseCode     A {@code int} representing the course within the department.
   *
   * @param count          A {@code int} representing the count of students enrolled
   *                       in the given course.
   *
   * @return               A {@code ResponseEntity} object containing an HTTP 200
   *                       response with an appropriate message or the proper status
   ...
   * This method handles PATCH requests to change the time
   * of a course identified by department code and course code.
